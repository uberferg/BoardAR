...
public class CannyActivity extends Activity implements CvCameraViewListener {
    /**
    ...
    private int loCannyThresh;
    private int hiCannyThresh;
    ...
    public Mat onCameraFrame(Mat aInputFrame) {
        ...
        // resize image before heavy calculations
        resizedFrame = StereoUtils.stereoResize(aInputFrame);

        greyFrame = new Mat(resizedFrame.rows(), resizedFrame.cols(), CvType.CV_8UC1);
        edgeFrame = greyFrame.clone();

        // convert to grayscale
        Imgproc.cvtColor(resizedFrame, greyFrame, Imgproc.COLOR_RGBA2GRAY);

        // Gaussian blur to reduce noise and false edges
        Imgproc.GaussianBlur(greyFrame, greyFrame, new Size(1, 1), 2, 2);

        // Perform Canny edge detection
        Imgproc.Canny(greyFrame, edgeFrame, loCannyThresh, hiCannyThresh);

        // reformat the output image to stereo vision for Google Cardboard
        aInputFrame = StereoUtils.stereoRender(edgeFrame, aInputFrame);
        System.gc();
        return aInputFrame;
    }
